/**
 * @file
 * Week7/HW/best-fit-malloc.c
 * 
 * @brief
 * Best Fitのmalloc
 * 
 * @note
 * - first-fit-malloc.cのコードを参考に、渡す領域の決め方を変えて実装
 * 
 */
...
/// @brief Best Fit方式のメモリ割り当てを行う関数
/// @param alloc_size 割り当てるメモリのサイズ
/// @return 割り当てたメモリ領域のポインタ
void *best_fit_malloc(size_t alloc_size) {
  /* 空き領域を探す(Best Fit) */
  ...
  if (!alloc_metadata) {
    ...
    return best_fit_malloc(alloc_size);
  }
