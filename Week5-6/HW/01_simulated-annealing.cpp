...
/// @brief 問題を貪欲法で解く関数
/// @param map<int, City> cities
/// @param int start スタート地点のindex
/// @return Route ret_route
Route solve_greedy(map<int, City>& cities, int& start_index) {
    /* デバッグ用出力 */
...
/// @param map<int, City> cities 入力データ
/// @param Route greedy_route 貪欲法で求めたRoute
/// @return Route output_route 焼きなまし法で改善されたRoute
Route solve_simulated_annealing(map<int, City>& cities, const Route& greedy_route) {
    /* 準備 */
